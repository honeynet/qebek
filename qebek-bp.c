...
bool qebek_bpt_init()
{
	...
	memset(qebek_bpt, 0, QEBEK_BP_MAX * sizeof(qebek_bp_slot*));
...
bool qebek_bp_remove(target_ulong address, target_ulong cr3, target_ulong ebp)
{
	...
	for(bp_slot = qebek_bpt[hash]; bp_slot != NULL; bp_slot = bp_slot->next)
	{
		if(bp_slot->breakpoint == address && bp_slot->cr3 == cr3 && bp_slot->ebp == ebp)
		{
...
qebek_bp_slot* qebek_bp_check(target_ulong address, target_ulong cr3, target_ulong ebp)
{
	...
	for(bp_slot = qebek_bpt[hash]; bp_slot != NULL; bp_slot = bp_slot->next)
	{
		if(bp_slot->breakpoint == address && 
				(bp_slot->cr3 == 0 || bp_slot->cr3 == cr3) &&
				(bp_slot->ebp == 0 || bp_slot->ebp == ebp)
		  )
			return bp_slot;
	}
